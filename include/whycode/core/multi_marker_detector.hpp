...
class MultiMarkerDetector {
  public:
    MultiMarkerDetector(int number_of_circles, int width, int height,
                        const DetectorParameters& parameters = DetectorParameters());
    ~MultiMarkerDetector(void);

    /* Main function to detect markers*/
    bool detectMarkers(ImageHandler& image_handler, bool reset = false, DebugImageManager* debug_manager = nullptr);
