#include "whycode/triangulate/two_marker_apriltag_triangulation.hpp"
...
TwoMarkerApriltagTriangulationNode::TwoMarkerApriltagTriangulationNode(ros::NodeHandle& nh, ros::NodeHandle& private_nh)
...
void TwoMarkerApriltagTriangulationNode::loadParameters() {
...
void TwoMarkerApriltagTriangulationNode::setupRosCommunication() {
    // Subscriber
    poses_subscriber_ = nh_.subscribe("/tag_detections", 1, &TwoMarkerApriltagTriangulationNode::posesCallback, this);

    // Publishers
    if (config_.publish_pose) {
        pose_publisher_ = nh_.advertise<geometry_msgs::PoseStamped>("two_marker_triangulation_april/pose", 1);
    }
    ...
    if (config_.enable_ground_truth_comparison) {
        ground_truth_subscriber_ =
                nh_.subscribe("/odom_ground_truth", 1, &TwoMarkerApriltagTriangulationNode::groundTruthCallback, this);
    }

    // Camera odometry publisher
    if (config_.publish_camera_odom) {
        camera_odom_publisher_ = nh_.advertise<nav_msgs::Odometry>("two_marker_triangulation_april/camera_odom", 1);
    }

    // Processing timer
    processing_timer_ = nh_.createTimer(ros::Duration(1.0 / PROCESSING_RATE),
                                        &TwoMarkerApriltagTriangulationNode::processingTimerCallback, this);
}

void TwoMarkerApriltagTriangulationNode::posesCallback(const apriltag_ros::AprilTagDetectionArray::ConstPtr& msg) {
    // Reset found flags
    marker1_.reset();
    marker2_.reset();
    ...
    // Iterate AprilTag detections
    for (const auto& det : msg->detections) {
        if (det.id.empty()) {
            continue;
        }
        const int tag_id = det.id[0];  // use first ID

        // Extract plain Pose from PoseWithCovarianceStamped
        const geometry_msgs::Pose& pose = det.pose.pose.pose;

        if (tag_id == config_.marker_id_1) {
            marker1_.whycode_id  = tag_id;
            marker1_.tracking_id = tag_id;
            marker1_.id_valid    = true;
            marker1_.pose        = pose;
            marker1_.found       = true;
            marker1_.last_seen   = current_time;
        } else if (tag_id == config_.marker_id_2) {
            marker2_.whycode_id  = tag_id;
            marker2_.tracking_id = tag_id;
            marker2_.id_valid    = true;
            marker2_.pose        = pose;
            marker2_.found       = true;
...
void TwoMarkerApriltagTriangulationNode::processingTimerCallback(const ros::TimerEvent& event) {
    ...
    if (result.is_valid) {
        ...
        //                   result.camera_plane_angle * 180.0 / M_PI);
    } else {
        ROS_WARN_THROTTLE(1.0, "2-Marker pose estimation failed: %s", result.error_message.c_str());
    }
}

TwoMarkerApriltagTriangulationNode::PoseEstimationResult
TwoMarkerApriltagTriangulationNode::estimatePose(const Eigen::Vector3d& P1, const Eigen::Vector3d& P2,
...
bool TwoMarkerApriltagTriangulationNode::areTwoMarkersValid() const {
    const auto current_time     = std::chrono::steady_clock::now();
...
void TwoMarkerApriltagTriangulationNode::groundTruthCallback(const nav_msgs::Odometry::ConstPtr& msg) {
    latest_ground_truth_   = *msg;
    ground_truth_received_ = true;
}

TwoMarkerApriltagTriangulationNode::CameraOdometry
TwoMarkerApriltagTriangulationNode::calculateCameraOdometry(const PoseEstimationResult& result) {
    CameraOdometry odom;
...
void TwoMarkerApriltagTriangulationNode::maybeInitAlignment(const Eigen::Matrix3d& R_mc, const Eigen::Vector3d& t_mc) {
    if (have_alignment_)
        return;
...
void TwoMarkerApriltagTriangulationNode::publishCameraOdometry(const CameraOdometry& odom) {
...
void TwoMarkerApriltagTriangulationNode::compareWithGroundTruth(const CameraOdometry& calculated_odom) {
...
double TwoMarkerApriltagTriangulationNode::normalizeAngle(double angle) const {
    while (angle > M_PI)
        angle -= 2.0 * M_PI;
    while (angle < -M_PI)
        angle += 2.0 * M_PI;
...
bool TwoMarkerApriltagTriangulationNode::validateInputs(const Eigen::Vector3d& P1, const Eigen::Vector3d& P2,
                                                        double known_distance) const {
...
TwoMarkerApriltagTriangulationNode::CameraPlaneAngles
TwoMarkerApriltagTriangulationNode::calculateCameraPlaneAngles(const Eigen::Vector3d& plane_normal) const {
...
void TwoMarkerApriltagTriangulationNode::publishTransform(const PoseEstimationResult& result) {
...
void TwoMarkerApriltagTriangulationNode::publishPose(const PoseEstimationResult& result) {
    geometry_msgs::PoseStamped pose_msg = eigenToRosPose(result.position, result.rotation);
    pose_publisher_.publish(pose_msg);
}

geometry_msgs::PoseStamped TwoMarkerApriltagTriangulationNode::eigenToRosPose(const Eigen::Vector3d& position,
...
void TwoMarkerApriltagTriangulationNode::spin() {
    ros::spin();
}

// Main function
int main(int argc, char** argv) {
    ros::init(argc, argv, "two_marker_triangulation_april_node");

    ros::NodeHandle nh;
    ros::NodeHandle private_nh("~");

    try {
        TwoMarkerApriltagTriangulationNode node(nh, private_nh);
        node.spin();
