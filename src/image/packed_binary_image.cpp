...
PackedBinaryImage::PackedBinaryImage(int width, int height) : width_(width), height_(height) {
    stride_ = (width + 7) / 8;  // Round up to nearest byte
    data_.resize(stride_ * height, 0);
}

bool PackedBinaryImage::findFirstBlackPixelInRow(int y, int start_x, int end_x, int& first_black_x) const {
    if (y < 0 || y >= height_)
        return false;
    ...
    for (int byte_idx = byte_start; byte_idx < byte_end && byte_idx < static_cast<int>(stride_); ++byte_idx) {
        ...
        if (hasByteBlackPixel(byte_value)) {
            ...
            for (int bit = (pixel_x < start_x) ? (start_x & 7) : bit_pos; bit < 8 && (byte_idx * 8 + bit) <= end_x;
                 ++bit) {
        ...
        for (; x + chunk_size <= width_; x += chunk_size) {
            for (int v = 0; v < vectors_per_chunk; v++) {
                ...
                if constexpr (simd_size == 64) {
                    ...
                    reinterpret_cast<uint64_t&>(dst_row[offset >> 3]) = static_cast<uint64_t>(bitmask);
                } else if constexpr (simd_size == 32) {
                    reinterpret_cast<uint32_t&>(dst_row[offset >> 3]) = static_cast<uint32_t>(bitmask);
                } else if constexpr (simd_size == 16) {
                    reinterpret_cast<uint16_t&>(dst_row[offset >> 3]) = static_cast<uint16_t>(bitmask);
                } else {
