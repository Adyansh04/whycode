...
bool compute_positions_scalar(Context& ctx) {
    ...
    for (int a = 0; a < id_samples; ++a) {
        ...
        if (A.x_coords[a] < 0 || width <= A.x_coords[a] || A.y_coords[a] < 0 || height <= A.y_coords[a]) {
            return false;
        }
    }
    return true;
...
bool compute_positions_simd_xsimd_trig_aligned(int width, int height, int N, const Segment& s,
...
bool compute_positions_simd_lut_trig_aligned(int width, int height, int N, const Segment& s,
                                             aligned_vec<float>& x_coords, aligned_vec<float>& y_coords,
                                             TrigLUTAligned& lut) {
    using bf     = xs::batch<float>;
    const int VS = bf::size;
...
int main() {
    ...
    bool           in_bounds_simd_trig_al =
            compute_positions_simd_xsimd_trig_aligned(width, height, id_samples, seg, x_simd_trig_al, y_simd_trig_al);

    bool in_bounds_simd_lut_al = compute_positions_simd_lut_trig_aligned(width, height, id_samples, seg, x_simd_lut_al,
                                                                         y_simd_lut_al, lut_al);

    // Correctness check vs scalar
    auto diff_stats = [&](auto& xs, auto& ys, const std::vector<float>& x0, const std::vector<float>& y0,
                          const char* name) {
        ...
        std::cout << std::left << std::setw(26) << name << " max|dx|=" << max_dx << " max|dy|=" << max_dy
                  << " RMS=" << rms << "\n";
    };
    ...
    auto bench_simd_trig_al = bench_once_vec("SIMD xsimd trig (aligned)", [&]() {
        ...
        return compute_positions_simd_xsimd_trig_aligned(width, height, id_samples, seg, x_simd_trig_al, y_simd_trig_al);
    });

    // 5) SIMD LUT trig (aligned)
    auto bench_simd_lut_al = bench_once_vec("SIMD LUT trig (aligned)", [&]() {
        return compute_positions_simd_lut_trig_aligned(width, height, id_samples, seg, x_simd_lut_al, y_simd_lut_al,
                                                       lut_al);
    });
    ...
    auto report = [](const BenchResult& br, double base_ms) {
        ...
        std::cout << std::left << std::setw(30) << br.name << " time=" << std::setw(10) << std::fixed
                  << std::setprecision(3) << br.ms << " ms"
                  << " CPU=" << std::setw(6) << std::setprecision(1) << br.cpu_percent << "%"
                  << " speedup=" << std::setprecision(2) << speedup << "x"
                  << " bounds=" << (br.in_bounds ? "ok" : "oob") << "\n";
    };

    std::cout << "=== Benchmark (id_samples=" << id_samples << ", SIMD width=" << xs::batch<float>::size << ") ===\n";
