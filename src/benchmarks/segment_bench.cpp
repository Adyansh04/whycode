...
void scalar_smooth_accumulation(
    const AnalysisData& analysis_data, int solution_idx, int id_samples, int segmentWidth,
...
void scalar_lut_smooth_accumulation(
    const AnalysisData& analysis_data, int solution_idx, int id_samples, int segmentWidth,
    const TrigLUTAligned& lut, float& sx, float& sy, int& num_points)
{
    sx         = 0.0f;
    sy         = 0.0f;
    num_points = 0;
...
void simd_lut_smooth_accumulation(
    const AnalysisData& analysis_data, int solution_idx, int id_samples, int segmentWidth,
    const TrigLUTAligned& lut, float& sx, float& sy, int& num_points)
{
...
void simd_ultra_lut_smooth_accumulation(
    const AnalysisData& analysis_data, int solution_idx, int id_samples, int segmentWidth,
    const TrigLUTAligned& lut, float& sx, float& sy, int& num_points)
{
    using bf             = xs::batch<float>;
...
int main()
{
    ...
    results.push_back(benchmark_function(
        "Scalar LUT",
        [&](float& sx, float& sy, int& num_points) {
            scalar_lut_smooth_accumulation(
                analysis_data,
                solution_idx,
                id_samples,
                segmentWidth,
    ...
    results.push_back(benchmark_function(
        "SIMD Ultra",
        [&](float& sx, float& sy, int& num_points) {
            simd_ultra_lut_smooth_accumulation(
                analysis_data,
                solution_idx,
                id_samples,
                segmentWidth,
    ...
    std::cout << std::left << std::setw(18) << "Method" << std::setw(12) << "Time (ms)"
              << std::setw(10) << "CPU %" << std::setw(10) << "Speedup" << std::setw(12) << "sx"
              << std::setw(12) << "sy" << std::setw(8) << "Points" << std::endl;
    ...
    for (const auto& result : results)
    {
        ...
        std::cout << std::left << std::setw(18) << result.name << std::setw(12) << std::fixed
                  << std::setprecision(3) << result.time_ms << std::setw(10) << std::setprecision(1)
                  << result.cpu_percent << std::setw(10) << std::setprecision(2) << speedup << "x"
                  << std::setw(12) << std::setprecision(6) << result.sx << std::setw(12)
                  << std::setprecision(6) << result.sy << std::setw(8) << result.num_points
                  << std::endl;
    }
    ...
    for (size_t i = 1; i < results.size(); ++i)
    {
        ...
        else
        {
            std::cout << "✗ FAIL - ";
            if (!sx_ok)
                std::cout << "sx diff=" << (results[i].sx - results[0].sx) << " ";
            if (!sy_ok)
                std::cout << "sy diff=" << (results[i].sy - results[0].sy) << " ";
            if (!points_ok)
                std::cout << "points diff=" << (results[i].num_points - results[0].num_points)
                          << " ";
            std::cout << std::endl;
            all_correct = false;
        }
    ...
    std::cout << "\nOverall: " << (all_correct ? "All tests PASSED" : "Some tests FAILED")
              << std::endl;
    ...
    std::cout << "✓ Loop unrolling with multiple accumulators (reduces dependency chains)"
              << std::endl;
    ...
    std::cout << "\nBest speedup achieved: " << std::fixed << std::setprecision(2) << best_speedup
              << "x over scalar baseline" << std::endl;

    std::cout << "\n=== PERF ANALYSIS COMMANDS ===" << std::endl;
    std::cout << "perf stat -e cycles,instructions,cache-references,cache-misses ./smooth_benchmark"
              << std::endl;
    std::cout << "perf stat -e "
                 "fp_arith_inst_retired.128b_packed_single,fp_arith_inst_retired.256b_packed_"
                 "single ./smooth_benchmark"
              << std::endl;
