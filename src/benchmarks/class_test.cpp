...
class Example {
public:
    Example() : s_(0), x_(1.0f), y_(2.0f) {}
    explicit Example(int seed) : s_(seed), x_(seed * 0.5f + 1.0f), y_(seed * 0.25f + 2.0f) {}
...
int main() {
    ...
    // Direct
    results.push_back(bench("direct.tick()", [&]() -> int64_t {
        Example obj(123);
        int64_t acc = 0;
    ...
    // unique_ptr
    results.push_back(bench("unique_ptr.tick()", [&]() -> int64_t {
    ...
    // Direct
    results.push_back(bench("direct.compute()", [&]() -> int64_t {
        Example obj(7);
        double sum = 0.0;
        for (int i = 0; i < inner_calls; ++i) {
    ...
    // unique_ptr
    results.push_back(bench("unique_ptr.compute()", [&]() -> int64_t {
    ...
    // Direct
    results.push_back(bench("direct.mix+compute", [&]() -> int64_t {
        Example obj(999);
        float acc = 0.0f;
        int64_t g = 0;
        for (int i = 0; i < inner_calls; ++i) {
    ...
    // unique_ptr
    results.push_back(bench("unique_ptr.mix+compute", [&]() -> int64_t {
    ...
    // Direct
    results.push_back(bench("direct.construct/destroy", [&]() -> int64_t {
    ...
    // unique_ptr
    results.push_back(bench("unique_ptr.construct/destroy", [&]() -> int64_t {
        int64_t g = 0;
        for (int i = 0; i < construct_loops; ++i) {
    ...
    auto find = [&](const std::string& name) -> int {
        for (size_t i = 0; i < results.size(); ++i) {
            if (results[i].name == name) return int(i);
        }
        return -1;
    };
    ...
    if (baseline_idx >= 0) {
        ...
        if (u >= 0) results[u].rel_speedup = base / results[u].ms;
    }
    ...
    std::cout << std::left << std::setw(28) << "Test"
              << std::setw(12) << "Time (ms)"
              << std::setw(10) << "CPU %"
              << std::setw(12) << "RelSpeed"
              << "Guard\n";
    std::cout << std::string(70, '-') << "\n";
    for (const auto& r : results) {
        std::cout << std::left << std::setw(28) << r.name
                  << std::setw(12) << std::fixed << std::setprecision(3) << r.ms
                  << std::setw(10) << std::setprecision(1) << r.cpu_percent
                  << std::setw(12) << std::setprecision(3) << r.rel_speedup
                  << r.guard << "\n";
    }

    std::cout << "\nNotes:\n";
    std::cout << "- For method calls, unique_ptr<T>->f() compiles to the same code as obj.f() after inlining; overhead mainly comes from heap allocation in construction and potential cache effects.\n";
    std::cout << "- The construct/destroy test isolates heap allocation overhead of std::make_unique vs. stack construction.\n";
    std::cout << "- Run with -O3 -march=native -DNDEBUG for realistic results. Consider taskset to pin CPU, and run multiple times.\n";
    std::cout << "- For perf: perf stat -e cycles,instructions,cache-references,cache-misses ./unique_vs_direct_bench\n";
    return 0;
}
