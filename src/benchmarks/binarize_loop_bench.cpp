...
int main()
{
    ...
    std::cout << "N = " << N << ", SIMD width = " << xs::batch<float>::size
              << ", iterations = " << iters << "\n\n";
    ...
    A_aligned.resize(N);
    for (int i = 0; i < N; ++i)
        A_aligned.signal[i] = A_scalar.signal[i];
    ...
    auto check = [&](const char* name, const std::vector<float>& s) {
        int diff = 0;
        for (int i = 0; i < N; ++i)
            diff += (int)(s[i] != A_scalar.smooth[i]);
    ...
    check(
        "SIMD aligned vs scalar  ",
        std::vector<float>(A_aligned.smooth.begin(), A_aligned.smooth.end()));
    ...
    auto r_simd_u = bench(
        ...
        [&]() { binarize_simd_unaligned(A_simd_u); },
        iters);
    auto r_simd_unroll = bench(
        "SIMD unrolled",
        [&]() { binarize_simd_unrolled(A_simd_unroll); },
        iters);
    auto r_simd_aligned = bench(
        "SIMD aligned",
        [&]() { binarize_simd_aligned(A_aligned); },
        iters);
    ...
    auto report = [&](const BenchResult& br, double base_ms) {
        ...
        std::cout << std::left << std::setw(16) << br.name << " time=" << std::setw(10)
    ...
    std::cout << "\nPerf tips:\n";
    std::cout
        << "perf stat -e cycles,instructions,cache-references,cache-misses ./simd_binarize_bench\n";
