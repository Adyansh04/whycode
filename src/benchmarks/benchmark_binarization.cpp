...
public:
    ...
    PackedBinaryImage(int w, int h) : width(w), height(h) {
...
public:
    ...
    PackedAlignedBinaryImage(int w, int h) : width(w), height(h) {
        stride = (w + 7) / 8; // round up to nearest byte
        data.resize(stride * h, 0);
    }
...
std::vector<uint8_t> loadImageAsGrayscale(const std::string& filename, int& width, int& height) {
    ...
    if (img.empty()) {
        std::cout << "Warning: Could not load image " << filename << ", using synthetic image instead." << std::endl;
        return {};
    }
...
aligned_vector<uint8_t> loadImageAsGrayscaleAligned(const std::string& filename, int& width, int& height) {
    cv::Mat img = cv::imread(filename, cv::IMREAD_COLOR);
    if (img.empty()) {
        std::cout << "Warning: Could not load image " << filename << ", using synthetic image instead." << std::endl;
        return {};
...
void saveVisualizationResults(const std::vector<uint8_t>& original_image, 
                             const std::vector<uint8_t>& binary_8bit,
                             const PackedBinaryImage& binary_optimized,
                             const PackedBinaryImage& binary_ultra_fast,
                             const PackedAlignedBinaryImage& binary_ultra_fast_aligned,
                             const PackedBinaryImage& binary_reduce,
...
std::vector<uint8_t> createTestPatternImage(int width, int height) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            float pattern3 = sqrt((x - width/2) * (x - width/2) + (y - height/2) * (y - height/2)) / (width/4); // Radial
...
void benchmark_simdlib_binarization(const std::vector<uint8_t>& src, int width, int height,
                                   uint8_t threshold, std::vector<uint8_t>& dst) {
    dst.resize(width * height);
    SimdBinarization(src.data(), width, width, height, threshold, 0, 255, 
                     dst.data(), width, SimdCompareGreater);
}

// Method 2: Highly Optimized SIMD with Proper Bitmask Usage
void benchmark_simd_binarization_optimized(const std::vector<uint8_t>& src, int width, int height,
                                          uint8_t threshold, PackedBinaryImage& dst) {
...
void benchmark_simd_binarization_ultra_fast(const std::vector<uint8_t>& src, int width, int height,
                                           uint8_t threshold, PackedBinaryImage& dst) {
    using batch_type = xs::batch<uint8_t>;
    constexpr size_t simd_size = batch_type::size;
...
void benchmark_simd_binarization_reduce(const std::vector<uint8_t>& src, int width, int height,
                                       uint8_t threshold, PackedBinaryImage& dst) {
    using batch_type = xs::batch<uint8_t>;
    constexpr size_t simd_size = batch_type::size;
...
void benchmark_simd_binarization_ultra_fast_aligned(const aligned_vector<uint8_t>& src, int width, int height,
                                                   uint8_t threshold, PackedAlignedBinaryImage& dst) {
    using batch_type = xs::batch<uint8_t>;
    constexpr size_t simd_size = batch_type::size;
...
BenchmarkResult run_benchmark(const std::function<void()>& func, const std::string& name, 
                             size_t memory_usage, int iterations = 1000) {
...
int main() {
    ...
    if (test_image.empty()) {
        ...
        std::cout << "Loaded image: " << image_path << " (" << width << "x" << height << ")" << std::endl;
    ...
    }, "Original SimdBinarization (8-bit)", width * height, iterations));
    ...
    }, "SIMD Optimized Bitmask", binary_optimized.getMemoryUsage(), iterations));
    ...
    }, "SIMD Ultra-Fast Chunked", binary_ultra_fast.getMemoryUsage(), iterations));
    
    // Benchmark 4: Reduce-based approach
    results.push_back(run_benchmark([&]() {
        benchmark_simd_binarization_reduce(test_image, width, height, threshold, binary_reduce);
    }, "SIMD Reduce-Based", binary_reduce.getMemoryUsage(), iterations));
    
    // Benchmark 5: NEW Ultra-fast with aligned memory access
    results.push_back(run_benchmark([&]() {
        benchmark_simd_binarization_ultra_fast_aligned(test_image_aligned, width, height, threshold, binary_ultra_fast_aligned);
    }, "SIMD Ultra-Fast Aligned", binary_ultra_fast_aligned.getMemoryUsage(), iterations));
    ...
    double best_speedup = baseline_time / std::min({results[1].time_ms, results[2].time_ms, results[3].time_ms, results[4].time_ms});
    
    std::cout << "Memory reduction: " << std::fixed << std::setprecision(1) 
              << memory_reduction << "% (" << memory_8bit << " → " << memory_1bit << " bytes)" << std::endl;
    std::cout << "Best speedup achieved: " << std::fixed << std::setprecision(2) << best_speedup << "x" << std::endl;
    std::cout << "Theoretical max speedup: " << xs::batch<uint8_t>::size << "x (SIMD width)" << std::endl;
    std::cout << "Efficiency: " << std::fixed << std::setprecision(1) 
              << (best_speedup / xs::batch<uint8_t>::size * 100) << "% of theoretical maximum" << std::endl;
    ...
    for (int i = 0; i < total_tests; i++) {
        ...
        if (binary_optimized.getPixel(x, y) == expected) correct_optimized++;
        if (binary_ultra_fast.getPixel(x, y) == expected) correct_ultra++;
        if (binary_ultra_fast_aligned.getPixel(x, y) == expected) correct_ultra_aligned++;
        if (binary_reduce.getPixel(x, y) == expected) correct_reduce++;
    }
    ...
    std::cout << "perf stat -e cycles,instructions,cache-references,cache-misses,branches,branch-misses,LLC-loads,LLC-load-misses ./final_benchmark" << std::endl;
    std::cout << std::endl;
    std::cout << "# SIMD instruction analysis:" << std::endl;
    std::cout << "perf stat -e fp_arith_inst_retired.128b_packed_single,fp_arith_inst_retired.256b_packed_single,fp_arith_inst_retired.512b_packed_single ./final_benchmark" << std::endl;
    std::cout << std::endl;
    std::cout << "# Memory bandwidth:" << std::endl;
    std::cout << "perf stat -e uncore_imc/data_reads/,uncore_imc/data_writes/ ./final_benchmark" << std::endl;
    ...
    saveVisualizationResults(test_image, binary_8bit, binary_optimized, 
                            binary_ultra_fast, binary_ultra_fast_aligned, binary_reduce, width, height, threshold);
