    ...
    int iterations)
{
    ...
    std::cout << "Average time per iteration: " << duration.count() / iterations << " ms" << std::endl;
    std::cout << "Result: num_points=" << data.num_points << ", sx=" << sx << ", sy=" << sy << std::endl;
    std::cout << "----------------------------------------" << std::endl;
}
...
int main() {
    ...
    std::cout << "======== BENCHMARKING WITH LOW BRANCH PREDICTION MISS RATE (10% change) ========" << std::endl;
    ...
    std::cout << "\n======== BENCHMARKING WITH HIGH BRANCH PREDICTION MISS RATE (50% change) ========" << std::endl;
    generate_data(data, id_samples, 0.50);
    run_benchmark("With Branch", process_with_branch, data, id_samples, segmentWidth, iterations);
    run_benchmark("Branch-Free", process_branch_free, data, id_samples, segmentWidth, iterations);
    
    std::cout << "\n======== BENCHMARKING WITH VERY LOW BRANCH PREDICTION MISS RATE (1% change) ========" << std::endl;
    generate_data(data, id_samples, 0.01);
    run_benchmark("With Branch", process_with_branch, data, id_samples, segmentWidth, iterations);
    run_benchmark("Branch-Free", process_branch_free, data, id_samples, segmentWidth, iterations);

    std::cout << "\n======== BENCHMARKING WITH VERY LOW BRANCH PREDICTION MISS RATE (0% change) ========" << std::endl;
