...
void compute_signal_original(
    const unsigned char* image_data, int width, int height, AnalysisData& analysis_data,
    int solution_idx, int id_samples, const ImageHandler& image_handler)
{
    float                gx, gy;
    int                  px, py, pos;
    ...
    for (int a = 0; a < id_samples; a++)
    {
        ...
        analysis_data.signal[a] = ptr[(pos + 0) * step + 0] * (1 - gx) * (1 - gy) +
                                  ptr[(pos + 1) * step + 0] * gx * (1 - gy) +
                                  ptr[(pos + width) * step + 0] * (1 - gx) * gy +
                                  ptr[step * (pos + width + 1) + 0] * gx * gy;
        analysis_data.signal[a] += ptr[(pos + 0) * step + 1] * (1 - gx) * (1 - gy) +
                                   ptr[(pos + 1) * step + 1] * gx * (1 - gy) +
                                   ptr[(pos + width) * step + 1] * (1 - gx) * gy +
                                   ptr[step * (pos + width + 1) + 1] * gx * gy;
        analysis_data.signal[a] += ptr[(pos + 0) * step + 2] * (1 - gx) * (1 - gy) +
                                   ptr[(pos + 1) * step + 2] * gx * (1 - gy) +
                                   ptr[(pos + width) * step + 2] * (1 - gx) * gy +
                                   ptr[step * (pos + width + 1) + 2] * gx * gy;
    }
}

// Level 1: Basic optimizations (hoisted calculations, grouped channel reads)
void compute_signal_optimized_basic(
    const unsigned char* image_data, int width, int height, AnalysisData& analysis_data,
    int solution_idx, int id_samples, const ImageHandler& image_handler)
{
    const unsigned char* ptr  = image_data;
...
void compute_signal_optimized_fma_prefetch(
    const unsigned char* image_data, int width, int height, AnalysisData& analysis_data,
    int solution_idx, int id_samples, const ImageHandler& image_handler)
{
    const unsigned char* ptr  = image_data;
    ...
    for (int a = 0; a < id_samples; ++a)
    {
        ...
        const float S00 =
            float(row0[px_step + 0]) + float(row0[px_step + 1]) + float(row0[px_step + 2]);
        const float S10 = float(row0[px_step + step + 0]) + float(row0[px_step + step + 1]) +
                          float(row0[px_step + step + 2]);
        const float S01 =
            float(row1[px_step + 0]) + float(row1[px_step + 1]) + float(row1[px_step + 2]);
        const float S11 = float(row1[px_step + step + 0]) + float(row1[px_step + step + 1]) +
                          float(row1[px_step + step + 2]);
...
void compute_signal_optimized_ultra(
    const unsigned char* image_data, int width, int height, AnalysisData& analysis_data,
    int solution_idx, int id_samples, const ImageHandler& image_handler)
{
    const unsigned char* ptr        = image_data;
    ...
    for (; a + 4 <= id_samples; a += 4)
    {
        ...
        for (int unroll = 0; unroll < 4; ++unroll)
        {
            ...
            const float S01 =
                float(base[width_step]) + float(base[width_step + 1]) + float(base[width_step + 2]);
            const float S11 = float(base[width_step + step]) + float(base[width_step + step + 1]) +
                              float(base[width_step + step + 2]);
...
int main()
{
    ...
    for (auto& p : image)
        p = static_cast<unsigned char>(pixel_dist(rng));
    ...
    results.push_back(benchmark_function(
        "Original",
        [&]() {
            compute_signal_original(
                image.data(),
                width,
                height,
                data_original,
    ...
    results.push_back(benchmark_function(
        "Basic Optimized",
        [&]() {
            compute_signal_optimized_basic(
                image.data(),
                width,
                height,
                data_basic,
    ...
    results.push_back(benchmark_function(
        "FMA + Prefetch",
        [&]() {
            compute_signal_optimized_fma_prefetch(
                image.data(),
                width,
                height,
                data_fma,
    ...
    results.push_back(benchmark_function(
        "Ultra Optimized",
        [&]() {
            compute_signal_optimized_ultra(
                image.data(),
                width,
                height,
                data_ultra,
    ...
    auto check_correctness = [&](const AnalysisData& test_data, const char* name) {
        ...
        std::cout << std::left << std::setw(20) << name << " max_diff=" << std::scientific
                  << std::setprecision(3) << max_diff << " rms=" << rms << std::endl;
    };
    ...
    std::cout << std::left << std::setw(20) << "Method" << std::setw(12) << "Time (ms)"
              << std::setw(12) << "CPU Usage" << std::setw(10) << "Speedup" << std::setw(15)
    ...
    for (const auto& result : results)
    {
        ...
        std::cout << std::left << std::setw(20) << result.name << std::setw(12) << std::fixed
                  << std::setprecision(3) << result.ms << std::setw(12) << std::setprecision(1)
                  << result.cpu_percent << "%" << std::setw(10) << std::setprecision(2) << speedup
                  << "x" << std::setw(15) << std::setprecision(1) << improvement << "%"
                  << std::endl;
    }

    // Compute and display average signal
    float avg = 0.0f;
    for (const auto& val : data_ultra.signal)
        avg += val;
    avg /= id_samples;
    ...
    std::cout
        << "perf stat -e cycles,instructions,cache-references,cache-misses,branches,branch-misses "
           "./bilinear_scalar_bench"
        << std::endl;
    std::cout << "perf stat -e L1-dcache-loads,L1-dcache-load-misses,LLC-loads,LLC-load-misses "
                 "./bilinear_scalar_bench"
              << std::endl;
    std::cout << "perf record -g --call-graph=dwarf ./bilinear_scalar_bench && perf report"
              << std::endl;
