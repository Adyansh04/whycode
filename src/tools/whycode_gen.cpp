...
std::string start_svg(const std::string size)
{
    ...
    ret += "<circle cx=\"900\" cy=\"900\" r=\"700\" fill=\"black\" stroke=\"none\"/>"
           "<path stroke=\"black\" d=\"M900,1800 L900,1775 M900,0 L900,25 M0,900 L25,900 M1800,900 "
           "L1775,900\"/>";
...
{
    return "<text x=\"1600\" y=\"100\" fill=\"black\" font-size=\"" + size + "\">" + txt +
           "</text>";
}
...
void draw_whycode_markers(const int id, const int idx, const int teethCount, const float w)
{
    ...
    std::cout << "Generating WhyCode for ID " << idx << " (encoding " << id << ") => " << idx
              << ".svg\n";
    ...
    {
        x1 = xc +
             650 * std::cos(-w * (2 * i + (s.at(i + 32 - teethCount) - '0') * 2.0) / 180.0 * M_PI);
        y1 = yc +
             650 * std::sin(-w * (2 * i + (s.at(i + 32 - teethCount) - '0') * 2.0) / 180.0 * M_PI);
        x2 = xc + 650 * std::cos(-w * (2 * i + 1) / 180.0 * M_PI);
        y2 = yc + 650 * std::sin(-w * (2 * i + 1) / 180.0 * M_PI);

        if (verbose)
        {
            std::cout << "Drawing Segment Size: " << x1 << "," << y1 << " " << x2 << "," << y2
                      << std::endl;
        }
...
int main(int argc, char* argv[])
{
    ...
    whycon::CNecklace decoder    = whycon::CNecklace(teethCount, hamming);
