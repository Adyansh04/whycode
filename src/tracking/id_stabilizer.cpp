...
IDStabilizer::IDStabilizer(int switch_threshold) : switch_threshold_(std::max(1, switch_threshold)) {
...
int IDStabilizer::stabilize(int tracking_id, int current_decoded_id) {
    ...
    if (inserted) {
        ...
        if (isValidID(current_decoded_id)) {
            WHYCON_INFO("[IDStabilizer] Initializing new track " << tracking_id << " with ID " << current_decoded_id);
...
inline bool IDStabilizer::isValidID(int id) const noexcept {
...
inline void IDStabilizer::decrementConfidenceForMismatch(IDHistory& history, int tracking_id,
                                                         int current_decoded_id) const {
    history.confidence_counter--;
    WHYCON_DEBUG("[IDStabilizer] Mismatch for track " << tracking_id << ". Locked: " << history.locked_id
...
inline void IDStabilizer::switchLockedID(IDHistory& history, int new_id, int tracking_id) const {
    WHYCON_INFO("[IDStabilizer] Track " << tracking_id << " switched from ID " << history.locked_id << " to " << new_id);
