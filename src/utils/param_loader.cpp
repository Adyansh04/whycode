...
ParamLoader::ParamLoader(const std::string& config_file_path) : config_file_path_(config_file_path) {
    try {
        ...
        std::cout << "[ParamLoader] Successfully loaded configuration from: " << config_file_path << std::endl;

    } catch (const YAML::Exception& e) { throw std::runtime_error("YAML parsing error: " + std::string(e.what())); }
}

YAML::Node ParamLoader::navigateNode(const std::string& category, const std::string& subcategory,
    ...
    if (!node[parameter]) {
        std::cerr << "[ParamLoader] Parameter not found: " << category << "/" << subcategory << "/" << parameter
                  << std::endl;
...
YAML::Node ParamLoader::navigateNode(const std::string& category, const std::string& parameter) const {
    ...
    if (!node[parameter]) {
        std::cerr << "[ParamLoader] Parameter not found: " << category << "/" << parameter << std::endl;
        throw std::runtime_error("Parameter not found: " + parameter);
    }
...
template <typename T>
T ParamLoader::getParams(const std::string& category, const std::string& subcategory,
                         const std::string& parameter) const {
    try {
        YAML::Node node;
        ...
        std::cout << "[ParamLoader] Successfully loaded " << category << "/" << parameter << ": " << node.as<T>()
                  << std::endl;
        ...
        std::cerr << "[ParamLoader] Bad conversion for parameter " << category << "/" << subcategory << "/" << parameter
                  << ": " << e.what() << std::endl;
        throw std::runtime_error("Parameter conversion failed");

    } catch (const std::exception& e) {
        std::cerr << "[ParamLoader] Using default value for " << category << "/" << parameter << std::endl;
        throw;
    }
}
...
template <typename T>
std::vector<T> ParamLoader::getParams(const std::string& category, const std::string& parameter, char delimiter) const {
    try {
        ...
        std::cerr << "[ParamLoader] Bad conversion for vector parameter " << category << "/" << parameter << ": "
                  << e.what() << std::endl;
...
bool ParamLoader::loadCameraIntrinsics(const std::string& camera_file_path, cv::Mat& camera_matrix,
    ...
    try {
        ...
        if (camera_config["camera_matrix"] && camera_config["camera_matrix"]["data"]) {
            std::vector<double> camera_data = camera_config["camera_matrix"]["data"].as<std::vector<double>>();
            if (camera_data.size() == 9) {
                ...
                std::cerr << "[ParamLoader] Invalid camera matrix size: " << camera_data.size() << std::endl;
                return false;
            }
        } else {
            std::cerr << "[ParamLoader] Camera matrix not found in " << camera_file_path << std::endl;
            return false;
        }

        // Load distortion coefficients
        if (camera_config["distortion_coefficients"] && camera_config["distortion_coefficients"]["data"]) {
            std::vector<double> dist_data = camera_config["distortion_coefficients"]["data"].as<std::vector<double>>();
            if (dist_data.size() >= 4 && dist_data.size() <= 8) {
                ...
                std::cerr << "[ParamLoader] Invalid distortion coefficients size: " << dist_data.size() << std::endl;
                return false;
            }
        } else {
            std::cerr << "[ParamLoader] Distortion coefficients not found in " << camera_file_path << std::endl;
            return false;
        }

        std::cout << "[ParamLoader] Successfully loaded camera intrinsics from: " << camera_file_path << std::endl;
        return true;

    } catch (const YAML::Exception& e) {
        std::cerr << "[ParamLoader] YAML error loading camera intrinsics: " << e.what() << std::endl;
        return false;
    } catch (const std::exception& e) {
...
template std::string ParamLoader::getParams<std::string>(const std::string&, const std::string&,
                                                         const std::string&) const;
template int         ParamLoader::getParams<int>(const std::string&, const std::string&, const std::string&) const;
template float       ParamLoader::getParams<float>(const std::string&, const std::string&, const std::string&) const;
template double      ParamLoader::getParams<double>(const std::string&, const std::string&, const std::string&) const;
template bool        ParamLoader::getParams<bool>(const std::string&, const std::string&, const std::string&) const;

// Two-parameter version (category, parameter) - ALL TYPES INCLUDING STRING
template std::string ParamLoader::getParams<std::string>(const std::string&, const std::string&) const;
...
template std::vector<double> ParamLoader::getParams<double>(const std::string&, const std::string&, char) const;
template std::vector<float>  ParamLoader::getParams<float>(const std::string&, const std::string&, char) const;
template std::vector<int>    ParamLoader::getParams<int>(const std::string&, const std::string&, char) const;
